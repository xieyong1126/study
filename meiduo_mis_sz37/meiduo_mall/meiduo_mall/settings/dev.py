...
#存数据时转成UTC时间,  比较时转成UTC时间去数据库进行比较
USE_TZ = True
...
CACHES = {
    "default": { # 默认存储信息: 存到 0 号库
        ...
        "LOCATION": "redis://192.168.208.129/0",
        "OPTIONS": {
    ...
    "session": { # session 信息: 存到 1 号库
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://192.168.208.129:6379/1",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    "verify_code": { # session 信息: 存到 1 号库
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://192.168.208.129:6379/2",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    "history": { # 存放用户浏览历史记录
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://192.168.208.129:6379/3",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    "carts": { # 存放用户浏览历史记录
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://192.168.208.129:6379/4",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
...
#http://image.meiduo.site:8888/group1/M00/00/02/wKjQgV8PG1CAWMNnAACZfYY_di88037694
...
JWT_AUTH = {
    ...
    'JWT_RESPONSE_PAYLOAD_HANDLER':'meiduo_admin.utils.jwt_reponse_handlers.custom_jwt_response_handler'
}